...
@section('content')
    <div class="row mt-minus">
        <div class="col">
            <div class="row">
                <div class="col">
                    <h5 class="page-heading text-light mb-4 mt-4 mt-md-0"><i class="material-icons">assignment</i>Total Transaction</h5>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row" style="margin-top: 2%;">
        <div class="col">
            <div class="bg-white rounded-lg py-5 px-5">
                <h2 id="tr" class="text-center">Total Transaction Report</h2>
                    ...
                    <form action = "" >
                        <div class="form-row">
                            ...
                            <div class="col d-none">
                                {{Form::select('account_type', $account_type, null, ['id' => 'account_type', 'class' => 'select2_op form-control','placeholder' => 'Select Type', 'required'])}}
                            </div>
...
@section('scripts')
    ...
    <script !src="">
        ...
        $(function() {
            var account_type = '';
            load_data();
            function load_data(from_date = '', to_date = '', account_type_id = '')
            {
                $('#trips_report_table').DataTable({
                    ...
                    buttons: [
                        {
                            extend: 'pdf',
                            messageTop: 'Total Transaction Report',
                            footer: true
                        },
                        'csv',
                        'excel',
                        {
                            extend: 'print',
                            messageTop: '<h2>Total Transaction Report ' + account_type + '</h2>',
                            footer: true
                        }
                    ],
                    ajax: {
                        url:'{!! route("get_account_month") !!}',
                        data:{from_date:from_date, to_date:to_date, account_type_id:account_type_id }
                    },
                    columns: [
                        {
                            title: "No",
                        ...
                        { data: 'note', name: 'note' },
                        { data: 'debit', name: 'debit', className: 'sum' },
                        { data: 'credit', name: 'credit', className: 'sum' },
            ...
            $('#filter').click(function(){
                ...
                var account_type_id = $("#account_type option:selected").val();
                console.log(account_type_id);
                if (account_type != ''){
                    account_type = '-'+ $("#account_type option:selected").text();
                    document.getElementById("tr").innerHTML = 'Total Transaction Report'+account_type;
                }else {
                    account_type = '';
                    document.getElementById("tr").innerHTML = 'Total Transaction Report'+account_type;
                }
                ...
                if( from_date != '' &&  to_date != '' )
                {
                    ...
                    load_data(from_date, to_date, account_type_id);
                }
                else
                {
            ...
            $('#refresh').click(function(){
                $('#from_date').val('');
                $('#to_date').val('');
                $("#account_type").select2().val('').trigger("change");
                $('#trips_report_table').DataTable().destroy();
                document.getElementById("tr").innerHTML = 'Total Transaction Report';
                load_data();
            });
